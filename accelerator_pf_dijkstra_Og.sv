...
/*-------------------------------------------------------------------------
        ...
        if (rst) begin
            ...
            case (cur_state)
                ...
                CHECK_NEIGHBORS: begin
                    ...
                    if (node_weight != 0 && !visited[neighbor_counter]) begin
                        if (temp_distance < distances[neighbor_counter]) begin
                            distances[neighbor_counter] <= temp_distance;
                            parent[neighbor_counter] <= current_node;
                        end
                    end
        ...
        case (cur_state)
            ...
            CHECK_NEIGHBORS: begin
                ...
                if (visited == 16'hFFFF) begin
                    next_state = SUCCESS;
                end else if (neighbor_counter == NODE_COUNT-1) begin
    ...
    -------------------------------------------------------------------------*/
    always_comb begin
            temp_distance = distances[current_node] + node_weight;
    end
    ...
    always_comb begin
        case (byte_sel)
            2'b00: node_weight = dijkstra_i_mem_data[NODE_WEIGHT_WIDTH-1:0];
            2'b01: node_weight = dijkstra_i_mem_data[2*NODE_WEIGHT_WIDTH-1:NODE_WEIGHT_WIDTH];
            2'b10: node_weight = dijkstra_i_mem_data[3*NODE_WEIGHT_WIDTH-1:2*NODE_WEIGHT_WIDTH];
            2'b11: node_weight = dijkstra_i_mem_data[4*NODE_WEIGHT_WIDTH-1:3*NODE_WEIGHT_WIDTH];
        endcase
    end
endmodule
